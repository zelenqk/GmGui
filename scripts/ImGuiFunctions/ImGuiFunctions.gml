...
function ImGuiFunctions() constructor {	
	#region general functions
	
	///@function deactivate()
	///@desc helper static to simply deactivate imguigml again
	...
	__ImguiIg(color_convert_gml_to_u32)
	
	///@function color_convert_gml_to_float4(_color, [_alpha=1.0]
	///@desc get an ImGui color4 from a gml color 
	///@param {Real} _color
	///@param {Real} [_alpha=1.0]   alpha value from 0 - 1
	///@returns {Array} [_r,_g,_b,_a]
	static color_convert_gml_to_float4 = function(_color, _alpha = 1) {
		var	_u32 = color_convert_gml_to_u32(_color, _alpha);
			return color_convert_u32_to_float4(_u32);
	}
	__ImguiIg(color_convert_gml_to_float4)
	
	///@function color_convert_float4_to_gml(_r_or_array4, [_g=undefined], [_b=undefined], [_a=undefined])
	...
	static color_convert_float4_to_gml = function() {
		...
		return color_convert_u32_to_gml(color_convert_float4_to_u32(_r, _g, _b, _a));
	}
	__ImguiIg(color_convert_float4_to_gml)
	
	///@function color_convert_u32_to_gml(_color)
	///@desc get gml color from an imgui color
	///@param {Real} _color   u32 color from imgui
	///@returns {Array:[ _gml_color, _alpha]}
	static color_convert_u32_to_gml = function(_col) {
	...
	__ImguiIg(color_convert_u32_to_gml)
	
	///@function toggle_cursor([_force])
	...
	static toggle_cursor = function() {
		var _forced = argument_count > 1 ? argument[0] : undefined;
	
		if (_forced == undefined) _forced = !Should_update_cursor;
		Should_update_cursor = _forced;
	}
	__ImguiIg(toggle_cursor)
	
	///@function enable_depth([_enabled])
	...
	static enable_depth = function() {
		var _enabled = argument_count > 0 ? argument[0] : undefined;
		if (_enabled == undefined) _enabled = Use_depth;
		else Use_depth = _enabled;
		
		Imgui_shader = Use_depth ? _EImGuiShader.Depth : _EImGuiShader.Normal;
	
		return _enabled;
	}
	__ImguiIg(enable_depth)
	
	///@function set_display_mode(_display_mode, [_display_view=0])
	...
	///      _If you're using `EImGui_DisplayMode.View` you may want to experiment with [set_display_scale](#set_display_scale)_
	...
	static set_display_mode = function() {
	...
	__ImguiIg(set_display_mode)
	
	///@function get_display_mode()
	///@desc returns the current set display mode
	///@returns {Real:EImGui_DisplayMode} ImGuIGML's current `Display_mode`
	static get_display_mode = function() {
		return Display_mode;
	}
	__ImguiIg(get_display_mode)
	
	///@function get_display_view()
	...
	///@returns {Real:EImGui_DisplayMode} ImGuIGML's current `Display_mode`
	static get_display_view = function() {
		return Display_view;
	}
	__ImguiIg(get_display_view)
	
	///@function set_display_scale([_x], [_y])
	...
	static set_display_scale = function() {
	...
	__ImguiIg(set_display_scale)
	
	///@function get_display_scale_width()
	///@desc get the scale % we scale the width of
	///@returns {Real}
	static get_display_scale_width = function() {
		return Scale_x;
	}
	__ImguiIg(get_display_scale_width)
	
	///@function get_display_scale_height()
	///@desc get the scale % we scale the height of
	///@returns {Real}
	static get_display_scale_height = function() {
		return Scale_y;
	}
	__ImguiIg(get_display_scale_height)
	
	///@function set_display_size([_width], [_height])
	///@desc set the display size (how imgui will clamp windows). 
	///      by default, ImGuiGML automatically sets the display size to the window size each step. using this static disables that behavior.
	///      calling this fucntion with **NO** params, re-enables that functionality.
	///@param {Real} [_width] 
	///@param {Real} [_height]
	static set_display_size = function() {
		//with (ImGuiGML) {
	...
	__ImguiIg(get_display_size)
	
	#endregion
	#region drawlist functions
	
	///@function drawlist_add_line(_x1, _y1, _x2, _y1, _color, [_thickness=1.0])
	...
	static drawlist_add_line = function() {
	...
	__ImguiIg(drawlist_add_line)
	
	///@function drawlist_add_rect(_x1, _y1, _x2, _y2, _col, [_rounding = 0.0], [_rounding_corner_flags = ~0], [_thickness = 1.0])
	...
	///@param {Real} [_thickness=1.0]
	static drawlist_add_rect = function() {
	...
	__ImguiIg(drawlist_add_rect)
	
	///@function drawlist_add_rect_filled(_x1, _y1, _x2, _y2, _col, [_rounding = 0.0], [_rounding_corner_flags = ~0])
	...
	///@param {Real} [_rounding=0.0]
	///@param {Real:EImGui_DrawFlags} [_rounding_corner_flags=0]
	static drawlist_add_rect_filled = function() {
		var _in = __Imgui_in, argi = 0;
		sr_buffer_write(_in, argument[argi++], ERousrData.Float);
		sr_buffer_write(_in, argument[argi++], ERousrData.Float);
		sr_buffer_write(_in, argument[argi++], ERousrData.Float);
	...
	__ImguiIg(drawlist_add_rect_filled)
	
	///@function drawlist_add_rect_filled_multicolor(_x1, _y1, _x2, _y2, _colUL, _colUR, _colBR, _colBL);
	...
	///@param {Real} _colBR
	///@param {Real} _colBL
	static drawlist_add_rect_filled_multicolor = function() {
		var _in = __Imgui_in, argi=0;
		sr_buffer_write(_in, argument[argi++], ERousrData.Float);
		sr_buffer_write(_in, argument[argi++], ERousrData.Float);
		sr_buffer_write(_in, argument[argi++], ERousrData.Float);
	...
	__ImguiIg(drawlist_add_rect_filled_multicolor)
	
	///@function drawlist_add_quad(_x1, _y1, _x2, _y2, _x3, _y3, _x4, _y4, _col, [_thickness = 1.0])
	...
	///@param {Real} _col
	///@param {Real} [_thickness=1.0]
	static drawlist_add_quad = function() {
		var _in = __Imgui_in;
	...
	__ImguiIg(drawlist_add_quad)
	
	///@function drawlist_add_quad_filled(_x1, _y1, _x2, _y2, _x3, _y3, _x4, _y4, _col, [_thickness = 1.0])
	...
	///@param {Real} _col
	///@param {Real} [_thickness=1.0]
	static drawlist_add_quad_filled = function() {
		var _in = __Imgui_in;
		sr_buffer_write(_in, argument[0], ERousrData.Float);
		sr_buffer_write(_in, argument[1], ERousrData.Float);
		sr_buffer_write(_in, argument[2], ERousrData.Float);
	...
	__ImguiIg(drawlist_add_quad_filled)
	
	///@function drawlist_add_triangle(_x1, _y1, _x2, _y2, _x3, _y3, _col, [_thickness=1.0])
	...
	///@param {Real} _col
	///@param {Real} [_thickness=1.0]
	static drawlist_add_triangle = function() {
		var in = __Imgui_in;
	...
	__ImguiIg(drawlist_add_triangle)
	
	///@function drawlist_add_triangle_filled(_x1, _y1, _x2, _y2, _x3, _y3, _col)
	...
	///@param {Real} _y3
	///@param {Real} _col
	static drawlist_add_triangle_filled = function() {
		var in = __Imgui_in;
		sr_buffer_write(in, argument[0], ERousrData.Float);
		sr_buffer_write(in, argument[1], ERousrData.Float);
		sr_buffer_write(in, argument[2], ERousrData.Float);
	...
	__ImguiIg(drawlist_add_triangle_filled)
	
	///@function drawlist_add_circle(_center_x, _center_y, _radius, _col, [_num_segments=12], [_thickness=1.0])
	...
	static drawlist_add_circle = function() {
		var in = __Imgui_in;
		sr_buffer_write(in, argument[0], ERousrData.Float);
		sr_buffer_write(in, argument[1], ERousrData.Float);
		sr_buffer_write(in, argument[2], ERousrData.Float);
	...
	__ImguiIg(drawlist_add_circle)
	
	///@function drawlist_add_circle_filled(_center_x, _center_y, _radius, _col, [_num_segments=12])
	///@desc Adds a circle to the drawlist
	///@param {Real} _center_x
	///@param {Real} _center_y
	///@param {Real} _radius
	///@param {Real} _col
	///@param {Real} [_num_segments=12]
	static drawlist_add_circle_filled = function() {
		var in = __Imgui_in;
		sr_buffer_write(in, argument[0], ERousrData.Float);
		sr_buffer_write(in, argument[1], ERousrData.Float);
		sr_buffer_write(in, argument[2], ERousrData.Float);
	...
	__ImguiIg(drawlist_add_circle_filled)
	
	///@function drawlist_add_text(_pos_x, _pos_y, _col, _text, _wrap_width) 
	...
	static drawlist_add_text = function() {
		var in = __Imgui_in, argi = 0;
		sr_buffer_write(in, argument[argi++], ERousrData.Float);
		sr_buffer_write(in, argument[argi++], ERousrData.Float);
		sr_buffer_write(in, argument[argi++], ERousrData.Uint32);
	...
	__ImguiIg(drawlist_add_text)
	
	///@function drawlist_add_surface(_surface, _x, _y, [_width=undefined], [_height=undefined], [_u0=0.0], [_v0=0.0], [_u1=1.0], [_v1=1.0], [color=c_white])
	...
	static drawlist_add_surface = function() {
		var argi = 0,
		...
		__drawlist_add_image(texture_id,
						//Coords
						x1, y1, x2, y2,
						//UVs
	...
	__ImguiIg(drawlist_add_surface)
	
	///@function drawlist_add_surface_quad(surface, _x1, _y1, _x2, _y2, x3, _y3, _x4, _y4, [_u1=0], [_v1=0], [_u2=1], [_v2=0], [_u3=1], [_v3=1], [_u4=0], [_v4=1], [_col=c_white])
	...
	///@param {Real} [_u4=0]
	///@param {Real} [_v4=1]
	static drawlist_add_surface_quad = function() {
		var argi = 0,
			surface = argument[argi++],
			x1 = argument[argi++],
			y1 = argument[argi++],
	...
	__ImguiIg(drawlist_add_surface_quad)
	
	///@function drawlist_add_sprite(_sprite_index, _sub_img, _x, _y, [_width=undefined], [_height=undefined], [_col=c_white])
	...
	///@param {Real} [_col=c_white]
	static drawlist_add_sprite = function() {
		...
		__drawlist_add_image(texture_id, x1, y1, x2, y2, u1, v1, u2, v2, color);
	}
	__ImguiIg(drawlist_add_sprite)
	
	///@function drawlist_add_polyline(_points, _col, [_closed=true], [_thickness=1.0], [_anti_aliased=true])
	...
	///@param {Real} [_thickness=1.0]
	static drawlist_add_polyline = function() {
		var in = __Imgui_in, argi = 0;
	...
	__ImguiIg(drawlist_add_polyline)
	
	///@function drawlist_add_convex_poly_filled(_points, _col, [_anti_aliased=true])
	///@desc add filled convex polygonal shape to the draw list
	///@param {Array:[_x,_y]} _points      an array of points, each point being an array: [x, y]
	///@param {Real} _col                   
	static drawlist_add_convex_poly_filled = function() {
		var in = __Imgui_in, argi = 0;
		var _points = argument[argi++],
			_num_points = array_length(_points);
		...
		//Optional AA
		if(argument_count > argi) sr_buffer_write(in, argument[argi++], ERousrData.Bool);
		else sr_buffer_write(in, 1, ERousrData.Bool);
		
		__imguigml_ext_call(_extImguiGML_drawlist_add_convex_poly_filled());
	}
	__ImguiIg(drawlist_add_convex_poly_filled)
	
	///@function drawlist_add_bezier_curve(_pos_x1, _pos_y1, _curve_x1, _curve_y1, _curve_x2, _curve_y2, _pos_x2, _pos_y2, _col, _thickness, [_num_segments=0]) 
	...
	static drawlist_add_bezier_curve = function() {
		var in = __Imgui_in, argi = 0;
		sr_buffer_write(in, argument[argi++], ERousrData.Float);
		sr_buffer_write(in, argument[argi++], ERousrData.Float);
		sr_buffer_write(in, argument[argi++], ERousrData.Float);
	...
	__ImguiIg(drawlist_add_bezier_curve)
	
	///@function drawlist_path_clear()
	///@desc Clears the current path
	static drawlist_path_clear = function() {
		__imguigml_ext_call(_extImguiGML_drawlist_path_clear());
	}
	__ImguiIg(drawlist_path_clear)
	
	///@function drawlist_path_line_to(_x, _y)
	///@desc add a line to the current path
	///@param {Real} _x
	///@param {Real} _y
	static drawlist_path_line_to = function() {
		var in = __Imgui_in;
		sr_buffer_write(in, argument[0], ERousrData.Float);
		sr_buffer_write(in, argument[1], ERousrData.Float);
		__imguigml_ext_call(_extImguiGML_drawlist_path_line_to());
	}
	__ImguiIg(drawlist_path_line_to)
	
	///@function drawlist_path_line_to_merge_duplicate(_x, _y)
	///@desc add a line to the current path, merging any duplicate lines this creates
	///@param {Real} _x
	///@param {Real} _y
	static drawlist_path_line_to_merge_duplicate = function() {
		var in = __Imgui_in;
		sr_buffer_write(in, argument[0], ERousrData.Float);
		sr_buffer_write(in, argument[1], ERousrData.Float);
		__imguigml_ext_call(_extImguiGML_drawlist_path_line_to_merge_duplicate());
	}
	__ImguiIg(drawlist_path_line_to_merge_duplicate)
	
	///@function drawlist_path_fill_convex(_col)
	///@desc add fill the current convex shape created bhy the path
	///@param {Real} _col
	static drawlist_path_fill_convex = function() {
		var in = __Imgui_in;
		sr_buffer_write(in, argument[0], ERousrData.Uint32);
		__imguigml_ext_call(_extImguiGML_drawlist_path_fill_convex());
	}
	__ImguiIg(drawlist_path_fill_convex)
	
	///@function drawlist_path_stroke(_col, _closed, [_thickness=1.0])
	///@desc set the stroke settings for drawing paths
	///@param {Real} _col
	///@param {Boolean} _closed
	///@param {Real} [_thickness=1.0])
	static drawlist_path_stroke = function() {
		var in = __Imgui_in;
		sr_buffer_write(in, argument[0], ERousrData.Uint32);
	...
	__ImguiIg(drawlist_path_stroke)
	
	///@function drawlist_path_arc_to(_center_x, _center_y, _radius, _min, _max, [_num_segments=10])
	...
	static drawlist_path_arc_to = function() {
		var in = __Imgui_in;
		var argi = 0;
		sr_buffer_write(in, argument[argi++], ERousrData.Float);
		sr_buffer_write(in, argument[argi++], ERousrData.Float);
	...
	__ImguiIg(drawlist_path_arc_to)
	
	///@function drawlist_path_arc_to_fast(_center_x, _center_y, _radius, _min_of_12, _max_of_12)
	...
	static drawlist_path_arc_to_fast = function() {
		var in = __Imgui_in;
		var argi = 0;
		sr_buffer_write(in, argument[argi++], ERousrData.Float);
		sr_buffer_write(in, argument[argi++], ERousrData.Float);
	...
	__ImguiIg(drawlist_path_arc_to_fast)
	
	///@function drawlist_path_bezier_curve_to(_x1, _y1, _x2, _y2, _x3, _y3, [_num_segments=0])
	///@desc Curve path to 
	///@param {Real} _x1 
	///@param {Real} _y1 
	///@param {Real} _x2 
	...
	///@param {Real} _y3 
	///@param {Real} [_num_segments=0]
	static drawlist_path_bezier_curve_to = function() {
		var in = __Imgui_in, argi = 0;
		sr_buffer_write(in, argument[argi++], ERousrData.Float);
		sr_buffer_write(in, argument[argi++], ERousrData.Float);
		sr_buffer_write(in, argument[argi++], ERousrData.Float);
	...
	__ImguiIg(drawlist_path_bezier_curve_to)
	
	///@function drawlist_path_rect(_x1, _x2, _y1, _y2, [_rounding=0.0], [_rounding_corner_flags=-1])
	...
	static drawlist_path_rect = function() {
		var in = __Imgui_in, argi = 0;
		sr_buffer_write(in, argument[argi++], ERousrData.Float);
		sr_buffer_write(in, argument[argi++], ERousrData.Float);
		sr_buffer_write(in, argument[argi++], ERousrData.Float);
		...
		sr_buffer_write(in, argument_count > argi ? argument[argi] : 0, ERousrData.Int32); argi++;
		if (!__imguigml_ext_call(_extImguiGML_drawlist_path_rect())) return;
	}
	__ImguiIg(drawlist_path_rect);
	
	///@function drawlist_channels_split(_channels_count)
	///@desc merge channels
	///      Channels:
	///      - Use to simulate layers. By switching channels to can render out-of-order (e.g. submit foreground primitives before background primitives)
	///      - Use to minimize draw calls (e.g. if going back-and-forth between multiple non-overlapping clipping rectangles, prefer to append into separate channels then merge at the end)
	///@param {Real} _channels_count 
	static drawlist_channels_split = function() {
		sr_buffer_write(__Imgui_in, argument[0], ERousrData.Int32);
		__imguigml_ext_call(_extImguiGML_drawlist_channels_split());
	}
	__ImguiIg(drawlist_channels_split)
	
	///@function drawlist_channels_merge()
	///@desc merge channels
	///      Channels:
	///      - Use to simulate layers. By switching channels to can render out-of-order (e.g. submit foreground primitives before background primitives)
	///      - Use to minimize draw calls (e.g. if going back-and-forth between multiple non-overlapping clipping rectangles, prefer to append into separate channels then merge at the end)
	static drawlist_channels_merge = function() {
		__imguigml_ext_call(_extImguiGML_drawlist_channels_merge());
	}
	__ImguiIg(drawlist_channels_merge)
	
	///@function drawlist_channels_set_current(_channel_index)
	///@desc merge channels
	///      Channels:
	///      - Use to simulate layers. By switching channels to can render out-of-order (e.g. submit foreground primitives before background primitives)
	///      - Use to minimize draw calls (e.g. if going back-and-forth between multiple non-overlapping clipping rectangles, prefer to append into separate channels then merge at the end)
	///@param {Real} _channel_index
	static drawlist_channels_set_current = function() {
		var in = __Imgui_in;
		sr_buffer_write(in, argument[0], ERousrData.Int32);
		__imguigml_ext_call(_extImguiGML_drawlist_channels_set_current());
	}
	__ImguiIg(drawlist_channels_set_current)
	
	///@function drawlist_push_clip_rect(_min_x, _min_y, _max_x, _max_y, [_intersect_with_clip_plane=false])
	...
	static drawlist_push_clip_rect = function() {
		var in = __Imgui_in;
		sr_buffer_write(in, argument[0], ERousrData.Float);
		sr_buffer_write(in, argument[1], ERousrData.Float);
		sr_buffer_write(in, argument[2], ERousrData.Float);
		...
		if (!__imguigml_ext_call(_extImguiGML_drawlist_push_clip_rect())) return;
	}
	__ImguiIg(drawlist_push_clip_rect)
	
	///@function drawlist_pop_clip_rect()
	///@desc End Render-level scissoring
	static drawlist_pop_clip_rect = function() {
		__imguigml_ext_call(_extImguiGML_drawlist_pop_clip_rect());
	}
	__ImguiIg(drawlist_pop_clip_rect)
	
	///@function drawlist_push_clip_rect_fullscreen()
	///@desc Push the clip rect for the fullscreen 
	static drawlist_push_clip_rect_fullscreen = function() {
		if (!__imguigml_ext_call(_extImguiGML_drawlist_push_clip_rect_fullscreen()))
			return;
	}
	__ImguiIg(drawlist_push_clip_rect_fullscreen);
	
	///@function drawlist_get_clip_rect_min()
	///@desc Gets the max pos of clipping rectangle
	///@returns {Array:[ _min_x, _min_y ]
	static drawlist_get_clip_rect_min = function() {
	...
	__ImguiIg(drawlist_get_clip_rect_min);
	
	///@function drawlist_get_clip_rect_max()
	///@desc Gets the max pos of clipping rectangle
	///@returns {Array:[ _max_x, _max_y ]
	static drawlist_get_clip_rect_max = function() {
	...
	__ImguiIg(drawlist_get_clip_rect_max)
	
	///@function drawlist_push_texture_id(_tex_id)
	...
	__ImguiIg(drawlist_push_texture_id)
	
	///@function drawlist_pop_texture_id()
	///@desc Pops texture from drawlist 
	static drawlist_pop_texture_id = function() {
		__imguigml_ext_call(_extImguiGML_drawlist_pop_texture_id());
	}
	__ImguiIg(drawlist_pop_texture_id)
	
	///@function drawlist_set_flags(_flags)
	///@desc set the antialising flags [per primitive]
	///@param {Real:EImGui_DrawListFlags} _flags
	static drawlist_set_flags = function(argument0) {
		var in = __Imgui_in;
		sr_buffer_write(in, argument0, ERousrData.Uint32);
		__imguigml_ext_call(_extImguiGML_drawlist_set_flags());
	}
	__ImguiIg(drawlist_set_flags)
	
	///@function drawlist_get_flags()
	///@desc get the antialising flags [per primitive]
	///@returns {Real:EImGui_DrawListFlags} _flags
	static drawlist_get_flags = function() {
		if (!__imguigml_ext_call(_extImguiGML_drawlist_set_flags()))
			return;
		return buffer_read(__Imgui_out, buffer_u32);
	}
	__ImguiIg(drawlist_get_flags)
	
	///@function drawlist_use_overlay()
	///@desc use imgui's overlay drawlist ontop of the screen
	static drawlist_use_overlay = function() {
		_extImguiGML_drawlist_use_overlay();
	}
	__ImguiIg(drawlist_use_overlay);
	
	///@function drawlist_use_window()
	///@desc use imgui's active window drawlist with drawlist functions
	static drawlist_use_window = function() {
		_extImguiGML_drawlist_use_window();
	}
	__ImguiIg(drawlist_use_window);
	
	///@function __drawlist_add_image_quad(_texture_id, _x1, _y1, _x2, _y2, x3, _y3, _x4, _y4, [_u1=0], [_v1=0], [_u2=1], [_v2=0], [_u3=1], [_v3=1], [_u4=0], [_v4=1], [_col=c_white])
	///@desc Adds an image quad to the draw list 
	///@param {Real} _texture_id created from _texture_id
	///@param {Real} _x1 
	///@param {Real} _y1 
	///@param {Real} _x2 
	///@param {Real} _y2
	...
	static __drawlist_add_image_quad = function() {
		var in = __Imgui_in, argi = 0;
	...
	__ImguiIg(__drawlist_add_image_quad)
	...
	///@param {Real} [_col=c_white]
	///@extensionizer { "docs": false }
	static __drawlist_add_image = function() {
		var in = __Imgui_in, argi = 0;
	...
	__ImguiIg(__drawlist_add_image)
	
	#endregion
	#region io functions
	
	///@function io_ini_saving_rate([_time])
	...
	static io_ini_saving_rate = function() {
		if(argument_count > 0){
			var in = __Imgui_in;
			sr_buffer_write(in, argument[0], ERousrData.Float);
			
			if (!__imguigml_ext_call(_extImguiGML_set_ini_saving_rate())) return;
			
			return argument[0];
		}
	
		if (!__imguigml_ext_call(_extImguiGML_get_ini_saving_rate())) return;
		
		var out = __Imgui_out;
		
		return buffer_read(out, buffer_f32);
	}
	__ImguiIg(io_ini_saving_rate)
	
	///@function io_mouse_double_click_time([_time])
	///@desc Time for a double-click, in seconds.
	///@param {Real} [_time]  sets time to use if passed (= 0.30)
	///@param returns { Real } current time to use 
	static io_mouse_double_click_time = function() {
		if(argument_count > 0){
			var in = __Imgui_in;
			sr_buffer_write(in, argument[0], ERousrData.Float);
		...
		if (!__imguigml_ext_call(_extImguiGML_get_mouse_double_click_time())) return;
			
		var out = __Imgui_out;
		return buffer_read(out, buffer_f32);
	}
	__ImguiIg(io_mouse_double_click_time)
	
	///@function io_mouse_double_click_max_dist([_distance])
	...
	static io_mouse_double_click_max_dist = function() {
		if(argument_count > 0){
			var in = __Imgui_in;
			sr_buffer_write(in, argument[0], ERousrData.Float);
			
			if (!__imguigml_ext_call(_extImguiGML_set_mouse_double_click_max_dist())) return;
			
			return argument[0];
		}
	
		if (!__imguigml_ext_call(_extImguiGML_get_mouse_double_click_max_dist())) return;
		
		var out = __Imgui_out;
		return buffer_read(out, buffer_f32);
	}
	__ImguiIg(io_mouse_double_click_max_dist)
	
	///@function io_mouse_drag_threshold([_distance])
	...
	static io_mouse_drag_threshold = function() {
		if(argument_count > 0){
			var in = __Imgui_in;
			sr_buffer_write(in, argument[0], ERousrData.Float);
			
			if (!__imguigml_ext_call(_extImguiGML_set_mouse_drag_threshold())) return;
			
			return argument[0];
		}
	
		if (!__imguigml_ext_call(_extImguiGML_get_mouse_drag_threshold())) return;
		
		var out = __Imgui_out;
		return buffer_read(out, buffer_f32);
	}
	__ImguiIg(io_mouse_drag_threshold)
	
	///@function io_key_repeat_delay([_time])
	...
	static io_key_repeat_delay = function() {
		if(argument_count > 0){
			var in = __Imgui_in;
			sr_buffer_write(in, argument[0], ERousrData.Float);
			
			if (!__imguigml_ext_call(_extImguiGML_set_key_repeat_delay())) return;
			
			return argument[0];
		}
	
		if (!__imguigml_ext_call(_extImguiGML_get_key_repeat_delay())) return;	
		
		var out = __Imgui_out;
		return buffer_read(out, buffer_f32);
	}
	__ImguiIg(io_key_repeat_delay)
	
	///@function io_key_repeat_rate([_time])
	...
	static io_key_repeat_rate = function() {
		if(argument_count > 0){
			var in = __Imgui_in;
			sr_buffer_write(in, argument[0], ERousrData.Float);
			
			if (!__imguigml_ext_call(_extImguiGML_set_key_repeat_rate())) return;
			
			return argument[0];
		}
	
		if (!__imguigml_ext_call(_extImguiGML_get_key_repeat_rate())) return;
		
		var out = __Imgui_out;
		return buffer_read(out, buffer_f32);
	}
	__ImguiIg(io_key_repeat_rate)
	
	///@function io_font_global_scale([_scale])
	///@desc Global scale all fonts
	///@param {Real} [_scale] sets scale for all fonts if passed (= 1.0)
	///@param returns { Real } current font scale
	static io_font_global_scale = function() {
		if(argument_count > 0){
			var in = __Imgui_in;
			sr_buffer_write(in, argument[0], ERousrData.Float);
			
			if (!__imguigml_ext_call(_extImguiGML_set_font_global_scale())) return;
			
			return argument[0];
		}
	
		if (!__imguigml_ext_call(_extImguiGML_get_font_global_scale())) return;
		
		var out = __Imgui_out;
		
		return buffer_read(out, buffer_f32);
	}
	__ImguiIg(io_font_global_scale);
	
	///@function io_font_allow_user_scaling([_boolean])
	...
	static io_font_allow_user_scaling = function() {
		if(argument_count > 0){
			var in = __Imgui_in;
			sr_buffer_write(in, argument[0], ERousrData.Bool);
			if (!__imguigml_ext_call(_extImguiGML_set_font_allow_user_scaling())) return;
			
			return argument[0];
		}
	
		if (!__imguigml_ext_call(_extImguiGML_get_font_allow_user_scaling())) return;
		
		var out = __Imgui_out;
		return (buffer_read(out, buffer_s8) != 0);
	}
	__ImguiIg(io_font_allow_user_scaling)
	
	///@function io_display_framebuffer_scaling([_x=1.0],[_y=1.0])
	...
	static io_display_framebuffer_scaling = function() {
		if(argument_count > 0){
			var in = __Imgui_in,
			...
			if (!__imguigml_ext_call(_extImguiGML_set_display_framebuffer_scaling())) return;
			
			return [_x, _y];
		}
	
		if (!__imguigml_ext_call(_extImguiGML_get_display_framebuffer_scaling())) return;
	...
	__ImguiIg(io_display_framebuffer_scaling)
	
	///@function io_osx_behaviors([_boolean])
	...
	///@param returns { Boolean } current value 
	static io_osx_behaviors = function() {
		if(argument_count > 0){
			var in = __Imgui_in;
			sr_buffer_write(in, argument[0] ? 1 : 0, ERousrData.Int8);
			
			if (!__imguigml_ext_call(_extImguiGML_set_osx_behaviors())) return;
			
			return argument[0];
		}
	
		if (!__imguigml_ext_call(_extImguiGML_get_osx_behaviors())) return;
		
		var out = __Imgui_out;
		return (buffer_read(out, buffer_s8)!=0);
	}
	__ImguiIg(io_osx_behaviors)
	
	///@function io_ini_filename([_path])
	...
	static io_ini_filename = function() {
		if (argument_count > 0) {
		  var in = __Imgui_in;
		  sr_buffer_write(in, argument[0], ERousrData.String);
		  
		  if (!__imguigml_ext_call(_extImguiGML_set_ini_filename())) return;
		  
		  return argument[0];
		} 
	
		if (!__imguigml_ext_call(_extImguiGML_get_ini_filename())) return;
		
		var out = __Imgui_out;
		return buffer_read(out, buffer_string);
	}
	__ImguiIg(io_ini_filename)
	
	///@function io_log_filename([_path])
	...
	static io_log_filename = function() {
		if (argument_count > 0) {
		  var in = __Imgui_in;
		  sr_buffer_write(in, argument[0], ERousrData.String);
		  
		  if (!__imguigml_ext_call(_extImguiGML_set_log_filename())) return;
		  
		  return argument[0];
		} 
	
		if (!__imguigml_ext_call(_extImguiGML_get_log_filename())) return;
		
		var out = __Imgui_out;
		return buffer_read(out, buffer_string); 
	}
	__ImguiIg(io_log_filename)
	
	///@function io_cursor_blink([_enabled])
	///@desc allow input currsors to blink
	///@param {Bool} [_enabled]   if passed, sets cursor blink to `_enabled`
	///@returns {Bool} 
	static io_cursor_blink = function() {
		if (argument_count > 0) {
		  var in = __Imgui_in;
		  sr_buffer_write(in, argument[0], ERousrData.Int8);
		  
		  if (!__imguigml_ext_call(_extImguiGML_set_cursor_blink())) return;
		  
		  return argument[0];
		} 
	
		if (!__imguigml_ext_call(_extImguiGML_get_cursor_blink())) return;
		
		var out = __Imgui_out;
		return buffer_read(out, buffer_string);
	}
	__ImguiIg(io_cursor_blink)
	
	#endregion
	#region mem functions
	
	///@function mem(key, [default_value])
	///@desc Get a value from the memory map
	///@param {String} key
	///@param {Real|String} [default_value] 
	///@returns {Real|String|undefined}
	static mem = function(_key, _default_value = undefined) {
		var _value = memory[$ _key];
	
		if (is_undefined(_value) && !is_undefined(_default_value)) {
			memory[$ _key] = _default_value;
	...
	///@function memset(_key, _val)
	///@desc Sets a value in the memory map
	///@param {String} key  
	///@param {*} _val
	///@returns {*} _val
	static memset = function(_key, _value) {
		memory[$ _key] = _value;
		return _value;
	}
	__ImguiIg(memset)
	
	///@function secure_load_mem(filename)
	///@desc loads imguigml memory from the filesystem
	///@param filename {String} the path to use 
	static secure_load_mem = function(_filename) {
		if (file_exists(_filename)){
			var file = file_text_open_read(_filename);
			var json = file_text_read_string(file);
			memory = json_parse(json); 
			file_text_close(file);
			
		}
	}
	__ImguiIg(secure_load_mem)
	
	///@function secure_save_mem(filename)
	///@desc saves imguigml memory onto the filesystem
	///@param filename {String} the path to use 
	static secure_save_mem = function(_filename) {
		var file = file_text_open_write(_filename);
		file_text_write_string(file, json_stringify(memory));
		file_text_close(file);
	}
	__ImguiIg(secure_save_mem)
